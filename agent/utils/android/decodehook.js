...
function base64ToHex(str) {
  for (
    var i = 0, bin = base64Decode(str.replace(/[ \r\n]+$/, "")), hex = [];
    i < bin.length;
    ++i
  ) {
...
function hook_decode() {
  Java.perform(function () {
    ...
    mac.update.overload("[B", "int", "int").implementation = function (
      a,
      b,
      c
    ) {
    ...
    cipher.update.overload("[B", "int", "int").implementation = function (
      a,
      b,
      c
    ) {
    ...
    rSAPublicKeySpec.$init.overload(
...
exports.hook_decode = hook_decode